...
More in [`docs/screenshots/`](docs/screenshots/): hero (desktop and 390 px), why-it-failed inspector, same-test proof, the unprotected/protected toggle, the student forensics sheet, receipt, AWS evidence, New test, My runs. They are regenerated by `scripts/ui_check.mjs` (below), never edited by hand.
...
Browser checks and the screenshots above, against the deployed site:

```bash
npm install --prefix scripts          # playwright-core; drives the Edge or Chrome already installed
node scripts/ui_check.mjs --mode check   # asserts the pages show what the API returns
node scripts/ui_check.mjs --mode shots   # regenerates docs/screenshots/
node scripts/ui_check.mjs --mode mobile  # no horizontal scroll at 390 px
node scripts/ui_check.mjs --site http://localhost:5173   # same checks against a dev server
```

Every expected value is read from `/overview` and `/runs` first and then compared with the rendered page, so the checks fail if the UI ever stops reflecting backend data. It also proves the replay advances when animation frames are suppressed (what a hidden tab does).
