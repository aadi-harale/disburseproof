...
def _send_response(event: dict[str, Any], context: Any, status: str, reason: str, data: dict[str, Any]) -> None:
    body = json.dumps(
        {
            "Status": status,
    ...
    with urllib.request.urlopen(request, timeout=10) as response:
        logger.info("custom resource response sent", extra={"status": status, "http_status": response.status})
...
def handler(event: dict[str, Any], context: Any) -> None:
    ...
    try:
        if event["RequestType"] in ("Create", "Update"):
            ...
            experiment = ExperimentService(repos.batches, repos.experiments).ensure_demo_experiment()
            data = {
