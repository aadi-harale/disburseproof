...
def test_phase_a_sends_each_duplicated_event_twice(
    ...
    events = plan_phase(run_id=RUN, phase=InjectionPhase.A, definition=definition, batch=batch,
...
def test_phase_b_sends_every_other_event_once(
    ...
    events = plan_phase(run_id=RUN, phase=InjectionPhase.B, definition=definition, batch=batch,
                        entitlements=entitlements)
...
def test_events_carry_the_business_identity(
    ...
    event = plan_phase(run_id=RUN, phase=InjectionPhase.A, definition=definition, batch=batch,
...
def test_injection_refuses_a_changed_batch(
    ...
    with pytest.raises(ConflictError):
        plan_phase(run_id=RUN, phase=InjectionPhase.A, definition=definition, batch=batch, entitlements=changed)
