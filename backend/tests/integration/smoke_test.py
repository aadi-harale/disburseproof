...
def api_url_from_stack(stack: str, region: str) -> str:
    ...
    outputs = boto3.client("cloudformation", region_name=region).describe_stacks(StackName=stack)[
        "Stacks"
    ][0]["Outputs"]
...
def start_run(api: Api, experiment_id: str, processor: str) -> str:
    for _ in range(60):
        status, body = api.call(
            "POST", "/runs", {"experiment_id": experiment_id, "processor": processor}
        )
        if status == 202:
...
def observed(run: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        **{
        ...
        "one_payment_per_entitlement": invariants.get("one_payment_per_entitlement", {}).get(
            "result"
        ),
...
def main() -> int:
    ...
    print(
        f"Demo experiment {experiment['experiment_id']}  fingerprint {experiment['fingerprint'][:16]}…"
    )
    ...
    for processor in ("vulnerable", "protected"):
        ...
        if run["status"] != "COMPLETED":
            failures.append(
                f"{processor}: run {run_id} ended {run['status']} "
        ...
        want_counts = (
            {"paid_once": 76, "paid_twice": 12, "unpaid": 12, "pending": 0}
            if processor == "vulnerable"
            else {"paid_once": 100, "paid_twice": 0, "unpaid": 0, "pending": 0}
        )
        if status != 200 or students["counts"] != want_counts:
            failures.append(
                f"{processor}: student grid counts {students.get('counts')} != {want_counts}"
            )
    ...
    for key in EXPECTED["vulnerable"]:
        cells = [
            str(observed(runs[p]).get(key)) if p in runs else "-"
            for p in ("vulnerable", "protected")
        ]
        print(key.ljust(34) + cells[0].ljust(38) + cells[1])
    for processor, run in runs.items():
        print(
            f"{processor}: run {run['run_id']}  duration {run.get('duration_ms')} ms  "
